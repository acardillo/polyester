...
def query(
    ...
    store_type: str = typer.Option(
        "vector", "--store", "-s", help="Store type: vector, graph, hybrid"
    ),
    top_k: int = typer.Option(5, "--top-k", "-k", help="Number of results to return"),
    data_path: str = typer.Option(
        "data/python_docs.json", "--data", "-d", help="Path to data file"
    ),
):
...
def index(
    ...
    console.print("[yellow]Note: In-memory store. Data will be lost when program exits.[/yellow]")
